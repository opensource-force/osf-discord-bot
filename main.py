...
async def on_ready():
    ...
    # your one way ticket to getting rate limited by discord
    # await bot.tree.sync()
...
async def submit(ctx, name: str = None, link: str = None, *, description: str = None):
    if not name or not description:
        await ctx.send(
            "**Error:** `name`, `link`, and `description` are required to submit a project. Please use the command in the following format:\n"
...
async def search(ctx, *, description: str = None):
    if not description:
        await ctx.send(
            "**Error:** A `description` is required to find a project which is suitable for you. Please use the command in the following format:\n"
...
async def loadCogs():
    ...
    await bot.load_extension(cogs[0])
